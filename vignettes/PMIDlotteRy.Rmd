...
BiocManager::install("PMIDlotteRy")
...
`r Biocpkg("PMIDlotteRy")` is based on many other packages and in particular in those that have implemented the infrastructure needed for dealing with RNA-seq data. That is, packages like `rentrez`.
...
We hope that `r Biocpkg("PMIDlotteRy")` will be useful for you. Please use the following information to cite the package and the overall approach. Thank you!
...
* `r Biocpkg("PMIDlotteRy")` `r Citep(bib[["PMIDlotteRy"]])`



# Reproducibility

The `r Biocpkg("PMIDlotteRy")` package `r Citep(bib[["PMIDlotteRy"]])` was made possible thanks to:
